...
async function fetcher([url, fetchOptions]: [string, RequestInit]): Promise<Tweet | null> {
 ...
 */
export const useTweet = (id?: string, apiUrl?: string, fetchOptions?: RequestInit) => {
  const { isLoading, data, error } = useSWR(
    () => (apiUrl || id ? [apiUrl || (id && `${host}/api/tweet/${id}`), fetchOptions] : null),
    fetcher,
    {
